...
namespace CommandLine {
    namespace detail {
        using argument_map = std::multimap<std::string_view, std::optional<std::string_view>>;
        ...
        struct parser {
            ...
            const argument_map& options() const {
                return options_;
            }
          ...
          private:
            ...
            void on_option(const std::string_view& option) {
                ...
                if (const auto delimiter = current_option_->find_first_of('='); delimiter != std::string_view::npos) {
        ...
        inline std::vector<std::string_view> get_values(const argument_map& options, const std::string_view& option) {
        ...
        template <>
        inline std::optional<std::string_view> get(const argument_map& options, const std::string_view& option) {
            return get_value(options, option);
        }

        template <>
        inline std::optional<std::string> get(const argument_map& options, const std::string_view& option) {
        ...
        // nullopt.
        template <class T>
        std::optional<T> get(const std::vector<std::string_view>& positional_arguments, size_t positional_index) {
        ...
        // Since the values are already stored as strings, there's no need to use `>>`.
        template <>
        inline std::optional<std::string_view> get(const std::vector<std::string_view>& positional_arguments, size_t positional_index) {
            if (positional_index < positional_arguments.size()) {
        ...
        template <>
        inline std::optional<std::string> get(const std::vector<std::string_view>& positional_arguments, size_t positional_index) {
            if (positional_index < positional_arguments.size()) {
